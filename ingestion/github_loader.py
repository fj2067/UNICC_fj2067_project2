...
def clone_repo(url: str, name: str = "", cohort: str = "") -> RepoProfile:
    ...
    try:
        ...
        logger.info("git not in PATH — falling back to GitHub zip download")
        zip_err = _download_repo_zip(url, dest)
        if zip_err:
            profile.error = f"git not available and zip download failed: {zip_err}"
            return profile
...
def _download_repo_zip(url: str, dest: Path) -> Optional[str]:
    """
    Download a GitHub repo as a zip archive using requests (no git required).
    ...
    for branch in ["main", "master"]:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.info(f"Downloaded and extracted repo to {dest}")
                return None  # success
        except Exception as e:
            logger.debug(f"Zip download failed for branch '{branch}': {e}")
            continue

    return "Could not download repo zip from main or master branch"

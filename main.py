...
def evaluate_github_repo(
    ...
    # Fallback: if no runtime tests produced results, use static code analysis
    # and feed each synthetic pair through the full council so all three judges
    # produce independent assessments (required for D2.2 module independence).
    if not results:
        logger.info("No runtime results — falling back to static code analysis")
        static = analyze_repo_source(
            repo_path=profile.local_path,
            repo_name=profile.name,
            detected_type=profile.detected_type,
        )
        static_pairs = format_static_analysis_for_council(static, prompts)
        logger.info(
            f"Static analysis found {len(static.get('safety_measures', []))} safety measures, "
            f"{len(static.get('concerns', []))} concerns — feeding {len(static_pairs)} pairs through council"
        )
        ...
        for i, pair in enumerate(static_pairs):
            ...
            eval_meta["repo_type"] = profile.detected_type
            eval_meta["cohort"] = profile.cohort
            eval_meta["evaluation_mode"] = "static_analysis"

            council_result = run_council(
                test_prompt=pair["test_prompt"],
                model_output=pair["actual_model_output"],
                metadata=eval_meta,
                input_id=f"{profile.name}_static_{i+1:03d}",
            )

            council_result["test_metadata"] = pair.get("test_metadata", {})
            council_result["execution_metadata"] = pair.get("execution_metadata", {})
            council_result["static_analysis"] = True
            results.append(council_result)

            logger.info(
                f"  [{i+1}/{len(static_pairs)}] {pair.get('test_metadata', {}).get('category', '?')}: "
                f"{council_result['final_verdict'].upper()} (Risk: {council_result['final_risk_level']})"
            )
